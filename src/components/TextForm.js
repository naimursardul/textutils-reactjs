...
const TextForm = (prop) => {
  ...
  const websiteExtractor = () => {
    ...
    const displayWebsites = (website) => {
      ...
      const d = ".com";
      if (
        website.toLowerCase().includes(b.toLowerCase()) &&
        website.toLowerCase().includes(d.toLowerCase())
      ) {
  ...
  const handleStrToText = () => {
    let newText = text
      .replace(/(?:^|\s)\d+(?=\s|$)/g, "")
      .replace(/\d{2}:\d{2}:\d{2},\d{3} --> \d{2}:\d{2}:\d{2},\d{3}/g, "") // Remove timestamps
      .replace(/\n+/g, " ") // Convert multiple newlines to a single space
    ...
    <>
      ...
      >
        ...
        <div className="mb-3">
          <textarea
            ...
            style={{
              ...
              backgroundColor: prop.mode === "dark" ? "#55485a" : "white",
            }}
          ></textarea>
        </div>
        ...
        <button
          disabled={text.length === 0}
          className="btn btn-primary mx-2 my-1"
          onClick={websiteExtractor}
        >
          Extarct websites url
        </button>
        <button
          disabled={text.length === 0}
          className="btn btn-primary mx-2 my-1"
